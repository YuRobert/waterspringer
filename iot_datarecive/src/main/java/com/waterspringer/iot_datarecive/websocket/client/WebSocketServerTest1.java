package com.waterspringer.iot_datarecive.websocket.client;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.waterspringer.commonutils.exceptionhandler.CarException;
import com.waterspringer.iot_datarecive.websocket.entity.LoginUser;
...
@Slf4j
@ServerEndpoint("/ws/{companyId}/{userId}")
@Component
public class WebSocketServerTest1
{
     ...
     */
    private static ConcurrentHashMap<LoginUser, WebSocketServerTest1> webSocketMap = new ConcurrentHashMap<>();
...
//    private String userId = "";
    ...
    @OnOpen
    public void onOpen(Session session, @PathParam("companyId") String companyId,@PathParam("userId") String userId) {
        this.session = session;
        this.loginUser = new LoginUser(userId,companyId);
     ...
     * @throws IOException
     */
    public static void sendInfo(String message, @PathParam("loginUser")LoginUser loginUser) throws IOException {
        ...
        while (entrys.hasNext()) {
            ...
            } else if (entry.getKey().equals(loginUser)) {
                webSocketMap.get(entry.getKey()).sendMessage(message);
                log.info("发送消息到：" + entry.getKey() + "，消息：" + message);
            }
    ...
    private static synchronized void addOnlineCount() {
        WebSocketServerTest1.onlineCount++;
    }
